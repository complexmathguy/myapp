...
public class DivisionTest{
		...
		try {            
			subscriber.divisionSubscribe().updates().subscribe(
					  successValue -> { 
						  ...
						  try {
							  divisionId = successValue.getDivisionId();
							  Division entity = read();
							  state( entity.getDivisionId() == successValue.getDivisionId(), "Subscribed value and associated read value are not equal");
						  } catch( Throwable exc ) {
			...
			subscriber.divisionSubscribe( divisionId ).updates().subscribe(
					  successValue -> { 
						  LOGGER.info(successValue.toString());
						  try {
							  List<Division> all = getAll();
							  int size = all.size();
							  divisionId = all.get(size-1).getDivisionId();
							  delete();
	...
	protected void delete() throws Throwable {
		...
		try{
			DivisionBusinessDelegate.getDivisionInstance().delete( new DeleteDivisionCommand( entity.getDivisionId() ) );
